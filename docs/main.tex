...
\begin{itemize}
    ...
    \item \textbf{inp\_max}: half-width of the interpolation grid in arcsec.
    \item \textbf{n\_points}: number of interpolation samples per spatial axis.
    \item \textbf{vec\_offsets}: velocity offsets in km/s relative to \texttt{vsys\_initial}.
\end{itemize}

\subsection{Channel maps}
\texttt{CHANNEL\_MAP\_PARAMS} controls output for channel-map plotters
\begin{itemize}
    \item \textbf{dv}: velocity spacing in km/s between plotted channels.
    \item \textbf{vlim}: maximum absolute velocity offset from systemic velocity.
    \item \textbf{plot\_lim}: spatial half-width in arcsec.
    \item \textbf{font\_size}, \textbf{axes\_labelsize}: matplotlib font sizes.
\end{itemize}

\subsection{3D HTML residual view}
\texttt{HTML\_PARAMS} controls the residual isosurface export:
\begin{itemize}
    \item \textbf{skip1}: spatial downsampling factor.
    \item \textbf{skip2}: spectral downsampling factor.
    \item \textbf{vlim}: velocity range used before meshing.
    \item \textbf{plot\_lim}: spatial limit used before meshing.
    \item \textbf{contour\_sigma}: multiplier used to set the isosurface threshold.
\end{itemize}

\subsection{Residual analysis}
\texttt{RESIDUAL\_ANALYSIS\_PARAMS} controls the post-processing maps:
\begin{itemize}
    \item \textbf{skip1}, \textbf{skip2}, \textbf{vlim}, \textbf{plot\_lim}: preprocessing controls.
    \item \textbf{min\_npix}: minimum connected voxel count for the dendrogram mask.
    \item \textbf{nsigma}: significance threshold in noise sigma.
\end{itemize}
